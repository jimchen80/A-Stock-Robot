...
def analyze_logic(row):
    try:
        ...
        elif energy_per_pct > 250000000: hidden_money = "对敲承接 | 压力沉重"
...
def run_task():
    ...
    try:
        ...
        # 强制插入时间行，解决内容不变更问题
        time_info = pd.DataFrame([[f"UPDATE_TIME: {bj_time}", "---实时更新---", "", "", "", "", "", random.random(), "", "", "", ""]], 
                                columns=['代码', '名称'] + cols)
        
        final_out = pd.concat([time_info, report[['代码', '名称'] + cols]], ignore_index=True)
        final_out.to_excel(file_name, index=False)
        print(f"✅ 文件已物理覆盖。")
